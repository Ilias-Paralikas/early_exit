...
class EarlyExitNet(nn.Module):
    def __init__(self,input_network,input_shape,thresholds,neurons_in_exit_layers = [[1024,1024],[1024,1024]]):
        ...
        self.network = deepcopy(input_network).to(device)
        x = torch.rand(input_shape).to(device)
        
        if isinstance(neurons_in_exit_layers,int):
            layers  =  neurons_in_exit_layers
            neurons_in_exit_layers = [[layers] for _ in range(len(self.network)-1)]
        self.output_shape  =  self.network(x).size(1)
        
        self.len = len(self.network)
        assert len(neurons_in_exit_layers) == len(self.network)-1

        self.exits = nn.ModuleList([])
        for i, net in enumerate(self.network[:-1]):
            layers = [nn.Flatten()]
        ...
        self.confidence_function = get_confidence_function()
    ...
    def forward(self,x):
        ...
        for i in range(self.len-1):
            ...
            early_exit = self.confidence_function(early_exit)
